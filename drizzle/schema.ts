...
// PostgreSQL Enums
export const userRoleEnum = pgEnum("user_role", ["admin", "manager", "assistant", "viewer"]);
export const poStatusEnum = pgEnum("po_status", ["pending", "partially_received", "completed", "cancelled"]);
export const reservationStatusEnum = pgEnum("reservation_status", ["active", "committed", "cancelled"]);
export const auditActionEnum = pgEnum("audit_action", ["create", "update", "delete"]);
export const notificationTypeEnum = pgEnum("notification_type", ["low_stock", "expiring_soon", "expired", "pending_order", "slow_moving"]);
export const surgeryCaseStatusEnum = pgEnum("surgery_case_status", ["planned", "materials_ready", "materials_partial", "in_progress", "completed", "cancelled"]);
export const materialStatusEnum = pgEnum("material_status", ["green", "yellow", "red"]);
export const caseMaterialStatusEnum = pgEnum("case_material_status", ["pending", "reserved", "used"]);
...
export const purchaseOrderItems = pgTable("purchaseOrderItems", {
  id: serial("id").primaryKey(),
  poId: integer("poId").notNull().references(() => purchaseOrders.id, { onDelete: "cascade" }),
  productId: integer("productId").notNull().references(() => products.id),
...
export const reservations = pgTable("reservations", {
  ...
  lotIdIdx: index("res_lotId_idx").on(table.lotId),
  statusIdx: index("res_status_idx").on(table.status),
  surgeryDateIdx: index("res_surgeryDate_idx").on(table.surgeryDate),
}));
...
export const usageLogs = pgTable("usageLogs", {
  ...
  notes: text("notes"),
  loggedBy: integer("loggedBy").notNull().references(() => users.id),
  createdAt: timestamp("createdAt").defaultNow().notNull(),
}, (table) => ({
  lotIdIdx: index("usage_lotId_idx").on(table.lotId),
  surgeryDateIdx: index("usage_surgeryDate_idx").on(table.surgeryDate),
}));
...
export const auditLogs = pgTable("auditLogs", {
  ...
  tableNameIdx: index("audit_tableName_idx").on(table.tableName),
  recordIdIdx: index("audit_recordId_idx").on(table.recordId),
 ...
 */
export const surgeryCases = pgTable("surgeryCases", {
  id: serial("id").primaryKey(),
  ...
  status: surgeryCaseStatusEnum("status").default("planned").notNull(),
  materialStatus: materialStatusEnum("materialStatus").default("red").notNull(),
  notes: text("notes"),
  createdBy: integer("createdBy").references(() => users.id),
  createdAt: timestamp("createdAt").defaultNow().notNull(),
  updatedAt: timestamp("updatedAt").defaultNow().notNull(),
}, (table) => ({
  caseNumberIdx: index("case_caseNumber_idx").on(table.caseNumber),
  surgeryDateIdx: index("case_surgeryDate_idx").on(table.surgeryDate),
 ...
 */
export const surgeryCaseMaterials = pgTable("surgeryCaseMaterials", {
  id: serial("id").primaryKey(),
  caseId: integer("caseId").notNull().references(() => surgeryCases.id, { onDelete: "cascade" }),
  productId: integer("productId").notNull().references(() => products.id),
  ...
  status: caseMaterialStatusEnum("status").default("pending").notNull(),
  reservationId: integer("reservationId").references(() => reservations.id),
  createdAt: timestamp("createdAt").defaultNow().notNull(),
  updatedAt: timestamp("updatedAt").defaultNow().notNull(),
});
...
export const pushSubscriptions = pgTable("pushSubscriptions", {
  ...
  userIdIdx: index("push_userId_idx").on(table.userId),
}));
